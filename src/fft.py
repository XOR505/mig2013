...
def fft(sig, sig2=[], meth="CT", mid=True, lin=True, freqMax=12000):
    """param sig : signal a traiter
        param sig2=[] : signal secondaire à calculer en parallele
    ...
    N = len(sig)
    #M = len(sig2) Need gestio exceptions
    if meth=="CT":
...
freq1,freq2 = 220,440
freqEch = 44100
s1 = [np.sin(2*np.pi*freq1*t/freqEch) for t in range(1000)]
s2 = [np.sin(2*np.pi*freq2*t/freqEch) for t in range(1000)]
C = fft(s1)
